...
func (run *RunMiddleware) ServerDrpFailoverHard(cmd *cobra.Command, args []string) {
	...
	// Validate and normalize target site
	...
	if !confirmed {
		fmt.Printf("%s\n", run.Colorize("⚠ DANGEROUS OPERATION", "red"))
		fmt.Printf("Hard failover will cause data loss.\n")
		fmt.Printf("Use --yes-i-understand-i-will-lose-data to confirm.\n")
		return
	}
...
func (run *RunMiddleware) ServerDrpResync(cmd *cobra.Command, args []string) {
	...
	// Validate and normalize authoritative site
	authoritativeSite = strings.ToLower(authoritativeSite)
	...
	if !confirmed {
		fmt.Printf("%s\n", run.Colorize("⚠ DANGEROUS OPERATION", "red"))
		fmt.Printf("Resync will OVERWRITE all data on the non-authoritative site.\n")
		fmt.Printf("The %s site will be used as the source of truth.\n", authoritativeSite)
		fmt.Printf("Use --yes-i-understand-i-will-lose-data to confirm.\n")
		return
	}
...
func (run *RunMiddleware) NetworkDrpDisable(cmd *cobra.Command, args []string) {
	...
	if !confirmed {
		...
		fmt.Printf("This may impact your disaster recovery capabilities.\n")
		fmt.Printf("Use --yes-i-understand-i-will-lose-data to confirm.\n")
		return
	}
