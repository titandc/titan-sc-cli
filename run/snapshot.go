...
func (run *RunMiddleware) SnapshotCreate(cmd *cobra.Command, args []string) {
	...
	if apiReturn != nil {
		...
		if !(apiReturn.Message == api.SnapshotCreateErrorLimitExceeded && forceErase) {
		...
		// Note: In legacy mode, we'd need server UUID to delete, but we only have one ID.
		// Since legacy create provides the UUID, delete should use the snapshot UUID from the list.
		// The OID field should contain the UUID when in legacy mode.
		if useLegacy {
			// In legacy mode, OID contains the UUID and we need server UUID for delete
			apiReturn, err = run.API.DeleteSnapshotLegacy(serverID, oldestSnapshot.OID)
		} else {
	...
	if !run.JSONOutput {
		...
		table := NewTable("NAME", "TIMESTAMP", "SIZE", "OID")
		table.SetNoColor(!run.Color)
		run.addSnapshotRow(table, &snapshot.Snapshot)
		table.Print()
		return
	}
...
func (run *RunMiddleware) SnapshotRotate(cmd *cobra.Command, args []string) {
	...
	if apiReturn != nil {
		...
		if apiReturn.Message != api.SnapshotCreateErrorLimitExceeded {
			run.printAPIReturn(apiReturn)
			return
		}
		...
		if !forceRotation {
			promptString := fmt.Sprint("This action will immediately delete snapshot '", oldestSnapshot.Name,
				"' (OID: ", oldestSnapshot.OID, ") created at ", DatePtrFormat(oldestSnapshot.CreatedAt), ".",
...
func (run *RunMiddleware) addSnapshotRow(table *Table, snap *api.Snapshot) {
