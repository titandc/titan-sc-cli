...
func (API *APITitan) ServerLoadISO(cmd *cobra.Command, args []string) {
	...
	_ = args
	API.ParseGlobalFlags(cmd)
	path, _ := cmd.Flags().GetString("url")
	serverUUID, _ := cmd.Flags().GetString("server-uuid")

	if path == "" {
		fmt.Println("Error: Load ISO option --url missing.")
		return
	}
	if serverUUID == "" {
		fmt.Println("Error: Load ISO option --server-uuid missing.")
		return
	}
	...
	err = API.SendAndResponse(HTTPPost, "/compute/servers/"+serverUUID+"/iso", reqData)
	if err != nil {
		fmt.Println(err.Error())
	}
	if !API.HumanReadable {
...
func (API *APITitan) ServerUnloadISO(cmd *cobra.Command, args []string) {
	...
	err := API.SendAndResponse(HTTPDelete, "/compute/servers/"+serverUUID+"/iso", nil)
	if err != nil {
		fmt.Println(err.Error())
	}
	if !API.HumanReadable {
